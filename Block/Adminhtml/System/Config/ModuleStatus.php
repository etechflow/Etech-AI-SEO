...
namespace Etechflow\AiSeo\Block\Adminhtml\System\Config;
...
/**
 * Status banner at the top of the AI SEO admin config section.
 *
...
class ModuleStatus extends Fieldset
{
    ...
    private function renderStatusBanner(): string
    {
        ...
        if (!$licenceValid) {
            if (!$hasKey) {
                return $this->banner(
                    ...
                    . 'The module is locked — the <strong>AI SEO Suggestions</strong> admin page redirects to the licence '
                    . 'gate, and meta generation (grid, mass-generate, apply, CLI) is disabled. Paste your key in the '
                    . '<strong>License Key</strong> field below, or visit '
                    . '<a href="' . $this->getUrl('aiseo/license/gate') . '" style="color:inherit;text-decoration:underline;">License &amp; Plans</a> '
                    . 'to purchase one.'
                );
            }
        ...
        return $this->banner(
            'success',
            '&#9989; Module is active',
            'Licence valid for <code>' . $this->escapeHtml($host) . '</code>. AI meta generation is enabled — '
            . 'generate and apply AI SEO titles/descriptions from the Suggestions grid, mass actions, and the CLI command.'
        );
    }
