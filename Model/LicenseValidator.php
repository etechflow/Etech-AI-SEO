...
/**
 * Portal license validator for Etechflow_AiSeo.
 * Follows PORTAL_LICENSING_GUIDE.md §3-step-1.
 *
 *   isValid() priority:
 *     1. revoked = 1                       → false (portal revoke wins even in dev)
 *     2. production_environment = No       → true (dev bypass — hardcoded off here)
 ...
 *   domain. The former "legacy HMAC" fallbacks (computeKey/computeBundleKey)
 *   derived a key from a secret that shipped in this file — anyone with the
 *   module could compute a valid key for their own domain — so they were
 *   removed. The former client-settable grace (issued_key/issued_at) was
 *   removed for the same reason: every input lived in admin config the customer
 *   controls. Offline grace now comes only from a cached genuine portal
 *   success, which the customer cannot fabricate.
 */
class LicenseValidator
{
    public const XML_PATH_LICENSE_KEY   = 'etechflow_aiseo/license/license_key';
    public const XML_PATH_PORTAL_URL    = 'etechflow_aiseo/license/portal_url';
    public const XML_PATH_PORTAL_API_URL = 'etechflow_aiseo/license/portal_api_url';
    ...
    private const MODULE_ID = 'ai-seo';

    private const CACHE_TTL_VALID  = 60;     // portal valid → 60s so admin IP-removal propagates within a minute
    private const CACHE_TTL_REJECT = 60;     // portal NO → recheck within a minute so re-authorisation propagates fast
    private const GRACE_TTL        = 172800; // 48h offline grace, refreshed on every portal success
    private const CACHE_TAG        = 'ETECHFLOW_AISEO';
    private const CACHE_PREFIX     = 'etf_aiseo_lic_';
    private const GRACE_PREFIX     = 'etf_aiseo_lic_grace_';
    ...
    public function isValid(): bool
    {
        ...
        // legacy-HMAC and client-grace paths were removed because every input
        // they trusted lived in admin config the customer controls.
        if (!str_starts_with($configuredKey, 'SP-')) {
            return false;
        }
    ...
    private function validateViaPortal(string $host, string $licenseKey): ?bool
    {
        ...
        try {
            ...
            $this->curl->addHeader('User-Agent', 'ETechFlow-AS/1.0');
            $this->curl->get($url);
    ...
    /**
     * Full portal /license/validate URL, used by the admin activation
     * controllers to derive the portal base for checkout redirects.
     */
    ...
    private function isExplicitlyRevoked(): bool
    {
        return (string) $this->scopeConfig->getValue(
            'etechflow_aiseo/license/revoked',
