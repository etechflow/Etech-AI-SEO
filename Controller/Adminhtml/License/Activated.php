...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        $block = $page->getLayout()->getBlock('etechflow.aiseo.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $planName)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_aiseo'))
                  ->setData('management_url', $this->getUrl('aiseo/suggestion/index'));
        }
